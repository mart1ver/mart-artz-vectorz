...
def run(duree=None, ip="127.0.0.1", sock=None):
    """Lance l'animation texte. duree=None → boucle infinie, sinon s'arrête après duree secondes."""
    _sock = sock if sock is not None else lxa.make_socket()
    dmx   = [0] * 512

    def set16(idx, val): lxa.set16(dmx, idx, val)
    def send():          lxa.send(_sock, dmx, ip)
    ...
    while True:
        t = time.time() - t0
        if duree is not None and t >= duree:
            break
        ...
        set16(7,  0);  set16(9,  0)
        set16(15, 0);  set16(17, 0)

        # ── PostFX
        dmx[22] = 2 if (mot_idx != 4 or int(t * 8) % 2 == 0) else 0
        dmx[23] = 255
        ...
        set16(base_tri + 9,  bg_size); set16(base_tri + 11, bg_size)
        set16(base_tri + 13, rot_tri)
        set16(base_tri + 15, 32767);   set16(base_tri + 17, 32767)
